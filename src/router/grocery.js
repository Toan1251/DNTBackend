...
router.get('/', groceryController.getGroceriesByName);

//get user groceries list
router.get('/user', passport.authenticate('jwt', { session: false }), groceryController.getUserGroceryList);
...
router.put('/update', passport.authenticate('jwt', { session: false }), upload.single('image'), groceryController.updateGrocery);

//update grocery on user groceries list
router.put('/update/:id', passport.authenticate('jwt', { session: false }), groceryController.updateUserGrocery);

//remove groceries from user groceries list
router.delete('/remove/:id', passport.authenticate('jwt', { session: false }), groceryController.removeGrocery);
